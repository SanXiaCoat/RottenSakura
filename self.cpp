...
void Player::combat(std::list<std::variant<enemy1*, enemy2*, enemy3*>>& enemies) 
{
    // 计算玩家中心位置
...
void Player::skill1(std::list<std::variant<enemy1_bullet*, enemy2_bullet*>>& enemyBullets)
{
    SDL_FPoint playerCenter = 
    {
        position.x + width / 2.0f,
    ...
    for (auto& bullet : enemyBullets) 
    {
        std::visit([this, &playerCenter](auto&& bullet) {
            ...
            if (distance <= meleeRadius * 2 && bullet->hasBeenSlowed == false) 
            {
            ...
            {
                bullet->speed = enemy1BulletSpeed;
...
void Player::skill2(std::list<std::variant<enemy1_bullet*, enemy2_bullet*>> &enemyBullets, std::list<std::variant<enemy1*, enemy2*, enemy3*>>& enemies)
{
    ...
    for(auto enemy : enemies)
    {
        std::visit([this, &playerCenter](auto&& enemy) {
            ...
            {
                enemy->health -= (1 + power / powerMax) * meleeDamage / 1.5f;
            }
        }, enemy);
    }
}
...
void Player::skill3(std::list<std::variant<enemy1_bullet*, enemy2_bullet*>> &enemyBullets,std::list<self_bullet *> &selfBullets) 
{
